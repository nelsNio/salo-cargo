...
@app.get("/review/frontend-config.js", include_in_schema=False)
def review_frontend_config():
    return FileResponse(FRONTEND_CONFIG, media_type="application/javascript")
...
@app.get("/review/{path:path}", include_in_schema=False)
def review_frontend(path: str = ""):
    return FileResponse(FRONTEND_INDEX)
...
def inbox_items():
    with db() as conn:
        rows = conn.execute("SELECT id,created_at,filename,status,draft_json,review_notes FROM inbox_items ORDER BY id DESC").fetchall()
    return [{**dict(row), "draft": json.loads(row["draft_json"] or "{}")} for row in rows]


@app.post("/api/inbox", status_code=201)
def create_inbox_item(item: InboxIn):
    with db() as conn:
        cur = conn.execute(
            "INSERT INTO inbox_items(created_at,filename,image_data,status,draft_json,review_notes) VALUES(?,?,?,?,?,?) RETURNING id",
            (date.today().isoformat(), item.filename, item.image_data, item.status, json.dumps(item.draft, ensure_ascii=False), item.review_notes),
        )
        return {"id": cur.fetchone()["id"], "filename": item.filename, "status": item.status, "draft": item.draft}


@app.put("/api/inbox/{item_id}")
def update_inbox_item(item_id: int, item: InboxUpdate):
    with db() as conn:
        cur = conn.execute(
            "UPDATE inbox_items SET status=?, draft_json=?, review_notes=? WHERE id=?",
            (item.status, json.dumps(item.draft, ensure_ascii=False), item.review_notes, item_id),
        )
        if cur.rowcount == 0:
